...
const DetailMonitoringVms = ({open, setOpen, openLoader, setOpenLoader}) => {
   ...
   const LihatDokumen = () =>{
    return(
        <Modal size="lg" show={modalDokumen} onHide={() => setModalDokumen(false)} className="fade" id="exampleModal" aria-labelledby="exampleModalLabel" aria-hidden="true">
            ...
            <Modal.Footer className="">
                <Button variant='contained' type="button" className="btn btn-primary" onClick={handleDownload} >Download</Button>
...
export default dynamic(() => Promise.resolve(DetailMonitoringVms), { ssr: false });